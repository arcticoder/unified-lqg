...
where $r_+ = M + \sqrt{M^2 - a^2}$ is the outer Kerr horizon. Representative values for $\mu = 0.1$ are:

\begin{table}[h]
\centering
\begin{tabular}{|c|c|c|}
\hline
\textbf{Spin $a$} & \textbf{$\Delta r_+(\mu=0.1)$} & \textbf{Fractional Shift} \\
\hline
$a=0.0$ & $-2.78 \times 10^{-4}$ & $-1.39 \times 10^{-4}$ \\
$a=0.5$ & $-2.63 \times 10^{-4}$ & $-1.14 \times 10^{-4}$ \\
$a=0.9$ & $-2.47 \times 10^{-4}$ & $-2.36 \times 10^{-4}$ \\
\hline
\end{tabular}
\caption{Kerr horizon shifts for $M=1$ using Bojowald prescription, which shows the most stable behavior across all spin values.}
\end{table}
